...
def _generate_request_id() -> str:
    """Generate a unique request id in the form ``timestamp-pid-random``.

    Used by the HTTP server to assign `HTTPServerRequest.request_id`.
    """
    return "%d-%d-%08x" % (time.time_ns(), os.getpid(), random.getrandbits(32))
...
class HTTPServerRequest:
    ...
    .. attribute:: request_id
       ...
       A unique identifier for this request, in the form
       ``timestamp-pid-random``.  It is generated by the server when the
       request is received and may be used to correlate log records (see
       `tornado.log.RequestIdFilter`) and is returned to the client in the
       ``X-Request-ID`` response header.  May be ``None`` for requests that
       were not received through `tornado.httpserver.HTTPServer` (for
       example in tests or WSGI applications).
    ...
    def __init__(
        ...
        if request_id is None and connection is not None:
            # Pick up the id generated by the server connection (if any).
            request_id = getattr(connection, "request_id", None)
