...
def generate_request_id() -> str:
    """Generates a unique identifier for an HTTP request.

    The id has the form ``<timestamp_ms>-<pid>-<random>``, e.g.
    ``1695000000000-12345-9f3a2c1e``.

    .. versionadded:: 6.6
    """
    return "%d-%d-%s" % (int(time.time() * 1000), os.getpid(), uuid.uuid4().hex[:8])
...
class HTTPServerRequest:
    ...
    .. attribute:: request_id
       ...
       A unique identifier for this request, of the form
       ``<timestamp_ms>-<pid>-<random>``. It is normally generated by the
       HTTP server when the request is received; if the request was not
       assigned an id by the server (or an explicit ``request_id`` was not
       passed to the constructor), one is generated automatically.
    ...
    def __init__(
        ...
        if request_id is None and connection is not None:
            # Pick up the id assigned by the server connection, if any.
